...
* **Visualize**: includes different ways to graphically represent data, such as tree, violin plot, scatter plot, bar plot, histograms, etc.
...
* **MECODA**: widgets that connect with data from different Citizen Science observatories (Minka, OdourCollect), data from DIY sensors such as CanAIRio, Aire Ciudadano or Smart Citizen.
...
* A widget can take as input the output of another widget and result in an output, which can communicate with a following widget.
...
* *Save Images*, a widget to save images on our computer. This widget only has curved line on the left side. That means that it accepts input, but it will carry out the process it is designed for (downloading the photos) and will not have any output in Orange.
...
We select the Minka widget and double click on the widget to display the dialog box. We select the geographical area that includes the location of the Prat town hall, which we can see here in Minka: https://minka-sdg.org/observations?place_id=246
...
The second thing we will do is select the time period that we want to be used to group the data (1 month, 1 year,...). There it will give us more or less columns. If the range of the data is very wide, it will allow us to create broad groupings (year). If the range is small, it will allow us to go down to the day or hour level.

The third thing we can do is divide our columns according to some criteria, for example, the realm to which they belong, the user who uploaded them, etc. If we choose a field with many options, the color division of the columns will make them difficult to read.
...
We see that we can use attributes such as the color of the points to view the data in our table. We could choose that the color of the points was based on the user (user_login column), the taxonomic class (class column), etc.
...
Or we can select all those that are not identified, to try to do it at a quick glance, using "Select Rows" and the condition *"taxon_name" is not defined*. This way we can see if we can identify any of the images. We simply select it, extract a table, and have the url of the observation in Minka to access it and identify it.
